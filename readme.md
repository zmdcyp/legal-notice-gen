...
- **Manual single（同步）**：`/generate_one` 直接返回 PDF 附件，`Content-Disposition` 用 RFC 5987 支持中文 / 乌尔都文等非 ASCII 文件名
...
| `POST` | `/generate_one` | Manual 模式：JSON `{template_slug, fields}` → PDF 流式附件（`Content-Disposition` 用 RFC 5987，非 ASCII 文件名友好） |
